...
function initUI(_box){
		...
		$(">li",$this).each(function(){
			...
			if($sub.length==1){
			    ...
			    if(mp==0){
					...
					var lis = $(li);
					var sub = lis.find("li");
					var a = lis.find("a[target=navTab],button[target=navTab]");
					var title = a.attr("title");
					var url = a.attr("href")
					var tabid = a.attr("rel") || "_newTab";
					var fresh = eval(a.attr("fresh") || "true");
					var external = eval(a.attr("external") || "false");
	...
	//打开一个窗口
	$("a[target=dialog2]", $p).each(function(){
		$(this).click(function(event){
			event.preventDefault();
			var $this = $(this);
			var url=$this.attr("href");
					...
					if(rows.length>0){
						...
						if (title) {
							Msg.confirm('提示',"当前一共勾选<span style='color:red'>[&nbsp;"+l+"&nbsp;]</span>行数据。<br/>"+title, function(r){
					               if (r){ 
					               var oo=MUI.openDialog(title,url,rel,options);
	...
	$("a[target=selectedMuilTodo]", $p).selectedMuilTodo();
