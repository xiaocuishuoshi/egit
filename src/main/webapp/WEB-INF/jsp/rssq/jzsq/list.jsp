	...
	<form onsubmit="return datagridSearch(this,'${param.rel }_datagrid');" >
		...
		<div class="search-content ">
			<span> <label>
				...
				<input type="text" name="ssbm" style="width: 90px"/> </span>
				<span> <label>
					借支金额：
				</label>
				<input type="text" name="sqje" style="width: 90px"/> </span>
		</div>
			...
			<span style="float: left;"> 
				<a class="easyui-linkbutton clearDgChecked"  icon="icon-redo"	plain="true"  title="清空所有勾选项" >清空勾选</a>
				 ...
				 <shiro:hasPermission name="public:add">
					<a class="easyui-linkbutton" icon="icon-add" plain="true" href="sqjz/load.do?rel=${param.rel }"
				  ...
				  <shiro:hasPermission name="public:update">
					<a class="easyui-linkbutton" icon="icon-edit" plain="true" title="修改"
						href="sqjz/update.do?id={id}&rel=${param.rel}"
				 ...
				 <shiro:hasPermission name="public:delete">
				 	...
				 	<a class="easyui-linkbutton" icon="icon-remove" plain="true"
							href="sqjz/delete.do" target="selectedTodo" rel="ids"
				...
				.datagrid(
						{
							...
							columns : [ [
									...
									},
									{
										...
										formatter : function(value, row, index) {
											...
											return '<a  datagrid="${param.rel }_datagrid" href="sqjz/show.do?id='
													+ row.id
													+ '" target="navTab"    rel="${param.rel}_find" title="借支申请详情">'
													+ value + '</a>';
										}
							...
							onDblClickRow:function(rowIndex, rowData){
								MUI.openDialog('修改请休假信息','sqjz/update.do?id='+rowData.id+'&rel=<%=request.getParameter("rel")%>','<%=request.getParameter("rel")%>_update',{width:1000,height:500});
							}
						});
