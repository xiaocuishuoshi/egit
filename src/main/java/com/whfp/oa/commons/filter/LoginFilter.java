 ...
 * @Package com.whfp.oa.commons.filter
...
public class LoginFilter implements Filter{
	...
	public void doFilter(ServletRequest arg0, ServletResponse arg1,FilterChain chain) throws IOException, ServletException {
		...
		System.out.println("请求1＝："+url+"   ====accept:"+accept); 
		String loginUser=request.getParameter("loginUserId");
		System.out.println("12222221111==loginUser=-"+loginUser);
		if(loginUser!=null){
			System.out.println("11111");
			service.updateLogin(loginUser, null, request, response);
		}else{
			try {
				String cookie=request.getHeader("cookie");
				  System.out.println("cookiess=="+cookie);
				...
				response.sendRedirect("http://219.138.150.221:8081/sso/checkLoadSso.do?sysFlag=A");
				System.out.println("33333333333333");
				String msg=HttpRequestUtil.request("http://219.138.150.221:8081/sso/sy_login/checkLoadSso.do?sysFlag=A", null, "post", "utf-8");
				msg=msg.replace("	", ""); 
           	//    ModelAndView mav2=JSON.toJavaObject(JSON.parseObject(msg), ModelAndView.class);// 
