...
public class BaseDaoImpl implements IBaseDao{
	...
	public Object findUniqueOne(String hql, List<Object> values) {
		...
		if (values != null && values.size() > 0) {
			for (int i = 0; i < values.size(); i++) {
				System.out.println(i+" value="+values.get(i));
				q.setParameter(i, values.get(i));
			}
		}
		return q.uniqueResult();
	...
	@Override
	public List findsql(String sql, List<SQLQueryMapping> typeList) {
		SQLQuery query = getSession().createSQLQuery(sql);
		if (typeList != null) {
			Iterator<SQLQueryMapping> keyIt = typeList.iterator();
			while (keyIt.hasNext()) {
				SQLQueryMapping mapping = keyIt.next();
				if (mapping != null) {
					query.addScalar(mapping.getName(), mapping.getType());
				}
			}
		}
		return query.list();
