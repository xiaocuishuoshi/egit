...
Connection conn=dataConn.Conn;
 Statement stmt=conn.createStatement();
  ...
  System.out.println("select city,sum(num)as num1  from(select city,num from "+City.tb+"_area_info_sn"+hh+" where   "+cond+" and not city='"+City.name+"') a group by city order by num1 desc limit 10 ");
  ResultSet rs=stmt.executeQuery("select city,sum(num)as num1  from(select city,num from "+City.tb+"_area_info_sn"+hh+" where   "+cond+" and not city='"+City.name+"') a group by city order by num1 desc limit 10 ");
 //'武汉': [112.20,32.08],
 ...
 while(rs.next()){
   ...
   System.out.println("select  max(num) from (select city,sum(num) as num  from (select city,num from "+City.tb+"_area_info_sn"+hh+" where  "+cond+" and not city='"+City.name+"')a group by city) b");
   rs=stmt.executeQuery("select  max(num) from (select city,sum(num) as num  from (select city,num from "+City.tb+"_area_info_sn"+hh+" where  "+cond+" and not city='"+City.name+"' )a group by city) b ");
   if(rs.next()){
  	 max=rs.getInt(1);
  }
  rs.close();
