 ...
 * @Project: jxoa
 * @Title: MyCache.java
 * @Package com.whfp.oa.commons.cache
...
import com.whfp.oa.commons.model.UserInfo;
...
public class MyCache {
	 ...
	 * 单位id-->名称
	 */
	public static final String ORGID2NAME="orgId2Name";
	...
	public String getOrgName(String ids){
		...
		Cache cache = ehcacheManager.getCache(MyCache.ORGID2NAME);
		String[] sz_ids=ids.split(",");
		StringBuffer names=new StringBuffer();
		for(String id:sz_ids){
			Element element = cache.get(id);
			if(element==null){
				SysOrg org=userService.get(SysOrg.class, id);
				if(org!=null){	
					cache.put(new Element(id,org.getOrgName()));
					names.append(org.getOrgName()+",");
				}
			}else{
