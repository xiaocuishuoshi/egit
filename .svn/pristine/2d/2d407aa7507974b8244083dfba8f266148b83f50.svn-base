...
<form method="post" action="saveTp.do"  onsubmit="return validateSubmitForm(this)">
	<table class="table table-bordered ">
	<input type="hidden" name="tpAddressId" value="${addss.id}"/>
	<input type="hidden" name="tpAddress" value="${addss.addressName}"/>
			<tr>
				<th>拓扑图名称：</th>
				<td  colspan="3">
				<input type="text" name="tpName" class="easyui-validatebox" required="true" />
				</td>
			</tr>
			<tr>
				<th style="width:80px">背景图：</th>
				<td  colspan="3">
					<select id="pathii" name="tpBack" onchange="changepic(this.value)" >
			...
			<tr>
				<th>虚拟号：</th>
				<td colspan="3">
					<input type="text" name="tpXnh" class="easyui-validatebox"  required="false" />
				</td>
			</tr>
