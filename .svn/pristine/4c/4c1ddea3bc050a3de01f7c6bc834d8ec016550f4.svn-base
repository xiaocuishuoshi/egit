			...
			<span style="float: left;"> 
				<a class="easyui-linkbutton clearDgChecked"  icon="icon-redo"	plain="true"  title="清空所有勾选项" >清空勾选</a>
				 ...
				 <shiro:hasPermission name="public:add">
					<a class="easyui-linkbutton" icon="icon-add" plain="true" href="cc/load.do?rel=${param.rel }"
				  ...
				  <shiro:hasPermission name="public:update">
					<a class="easyui-linkbutton" icon="icon-edit" plain="true" title="修改"
						href="cc/update.do?id={id}&rel=${param.rel}"
				 ...
				 <shiro:hasPermission name="public:delete">
				 	...
				 	<a class="easyui-linkbutton" icon="icon-remove" plain="true"
							href="cc/delete.do" target="selectedTodo" rel="ids"
				...
				.datagrid(
						{
							...
							columns : [ [
									...
									},
									{
										...
										formatter : function(value, row, index) {
											...
											return '<a  datagrid="${param.rel }_datagrid" href="cc/show.do?id='
													+ row.id
													+ '" target="navTab"    rel="${param.rel}_find" title="出差申请详情">'
													+ value + '</a>';
										}
							...
							onDblClickRow:function(rowIndex, rowData){
								MUI.openDialog('修改请休假信息','cc/update.do?id='+rowData.id+'&rel=<%=request.getParameter("rel")%>','<%=request.getParameter("rel")%>_update',{width:1000,height:500});
							}
						});
