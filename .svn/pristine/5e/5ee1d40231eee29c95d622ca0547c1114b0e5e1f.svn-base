...
<!--
	$(function(){
		$.ajax({
			url:"district/lookUp.do?superId=${superId}",
			dataType:"json",
			success:function(json){
				...
				$.each(json, function(i,d) {
					if(d.superId){
						zNodes.push({id : d.id,name : d.name,pId : d.superId,disSort:d.disSort,open:false});
					}
	...
	function chooseDistrict(event,treeId, treeNodeJSON){
		...
		names.reverse();
		$.bringBack({name:names.join("-"),disSort:treeNodeJSON.disSort});
